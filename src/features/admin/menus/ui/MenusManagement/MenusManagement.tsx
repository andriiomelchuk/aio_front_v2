...
export const MenusManagement = () => {
  ...
  const remove = async (menu: T_Menu) => {
    ...
    try {
      ...
      setError(t("admin.menus.error.deleteFailed"));
    }
  };

  const duplicate = async (menu: T_Menu) => {
    try {
      const copy = await duplicateMenu(menu.id);
      router.push(`/admin/menus/${copy.id}/edit`);
    } catch {
      setError(t("admin.menus.error.duplicateFailed"));
    }
  };
      ...
      <AdminCard title={t("admin.menus.pageTitle")} description={t("admin.menus.description", { total: menus.length })}>
        {isLoading ? (
          ...
          <div className="divide-y divide-border">
            {menus.map((menu) => (
              <article key={menu.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center">
                <div className="min-w-0 flex-1">
                  <h2 className="font-semibold text-foreground">{menu.name}</h2>
                  <p className="mt-1 text-sm text-muted">{menu.key} · {t(`admin.menus.status.${menu.status}`)} · {t("admin.menus.itemCount", { count: menu.items.length })}</p>
                </div>
                {canManage && (
                  <div className="flex flex-wrap gap-2">
                    <Button variant="secondary" onClick={() => router.push(`/admin/menus/${menu.id}/edit`)}>{t("admin.menus.actions.edit")}</Button>
                    <Button variant="secondary" onClick={() => void duplicate(menu)}>{t("admin.menus.actions.duplicate")}</Button>
                    <Button variant="danger" onClick={() => void remove(menu)}>{t("admin.menus.actions.delete")}</Button>
                  </div>
                )}
              </article>
            ))}
